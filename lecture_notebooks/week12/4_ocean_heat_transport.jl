...
\approx 
u_{i,\, j}^{n} \frac{T_{i+1,\, j}^{n} - T_{i-1,\,j}^{n}}{2 \Delta x} + v_{i,\, j}^{n} \frac{T_{i,\, j+1}^{n} - T_{i,\,j-1}^{n}}{2 \Delta y}.$
...
CFL_adv(sim::ClimateModelSimulation) = 
...
# ╔═╡ c1618e10-42d7-11eb-09aa-5358986ab10c
CFL_adv(ocean_sim)
