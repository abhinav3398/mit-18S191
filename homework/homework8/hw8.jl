...
begin
	...
	function interact(photon::Photon, hit::Intersection{Sphere}, num::Int64, obj::Vector{Object})
		ℓ₁ = photon.l
		old_ior = photon.ior
		sphere = hit.object
		p = hit.point
		n̂ = sphere_normal_at(p, sphere)
		coloured_ray=RGBA(0,0,0,0)
		reflectivity_index=sphere.s.r
		refractivity_index=sphere.s.t
		α=1-reflectivity_index-refractivity_index
		
		num==1 && (return Photon(p, ℓ₁, sphere.s.c, old_ior))
		
		if reflectivity_index ≉ 0
			ℓ₂=reflect(ℓ₁,n̂)
			reflected_ray=step_ray(Photon(p,ℓ₂,photon.c,old_ior),obj,num-1)
			coloured_ray += reflectivity_index*reflected_ray.c
		end

		if refractivity_index ≉ 0
			new_ior=old_ior==sphere.s.ior ? 1.0 : sphere.s.ior
			ℓ₂=refract(ℓ₁,n̂,old_ior,new_ior)
			refracted_ray=step_ray(Photon(p,ℓ₂,photon.c,new_ior),obj,num-1)
			coloured_ray += refractivity_index*refracted_ray.c
		end

		α ≉ 0 && (coloured_ray += α*sphere.s.c)

		Photon(p, ℓ₁, coloured_ray, old_ior)
	end
end
...
let
	scene = [sky]
	ray_trace(scene, basic_camera; num_intersections=3)
end
...
begin
	Echer = download("https://www.researchgate.net/profile/Madhu_Gupta22/publication/3427377/figure/fig1/AS:663019482775553@1535087571714/A-self-portrait-of-MC-Escher-1898-1972-in-spherical-mirror-dating-from-1935-titled.png") |> load
	Echer = convert(Array{RGB{Float64},2}, Echer)
end
...
# ╔═╡ 94008a40-2737-11eb-2a82-bdd0de845c1b
frame = imfilter([RGB((300 - y)/300)*rand() for y ∈ 1:300, x ∈ 1:600], Kernel.gaussian(2));

# ╔═╡ 6480b85c-2067-11eb-0262-f752d306d8ae
function padded(img)
	padded_img = copy(frame)
	# padded_img = copy(Echer)
	pad_x, pad_y = 5, 120
	frame_m, frame_n = size(frame)
	
	padded_img[1+pad_x:end-pad_x, 1+pad_y:end-pad_y] = imresize(img, (frame_m-2pad_x, frame_n-2pad_y))
...
# ╔═╡ 9769e540-273d-11eb-1b4b-63dbed64b9c0
frame |> size, face |> size
