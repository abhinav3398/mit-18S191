...
# ╔═╡ dada9530-4abb-11eb-2133-1b7925a1414c
pwd()
...
CFL_advection(model::ClimateModel) = 
...
# ╔═╡ c1ece740-435d-11eb-3c2d-894ccd83d99d
20000/.5

# ╔═╡ 0e85eb60-4359-11eb-2b9f-25118f845a48
md"""

$\mathcal{C}(t) = \mathcal{C}(2020) * 2^{(t-2020)/2}$
$\frac{\mathcal{C}(t)}{\mathcal{C}(2020)} = 2^{(t-2020)/2}$
$2*log_2(\frac{\mathcal{C}(t)}{\mathcal{C}(2020)}) = t-2020$
$t=2*log_2(\frac{\mathcal{C}(t)}{\mathcal{C}(2020)})+2020$
"""

# ╔═╡ 299d5540-2e6a-11eb-2698-05e889127454
cloud_resolution_possible_at = let
	# Nₓ = L/Δx ⟹ Nₓ ∝ 1/Δx
	floor(Int,2*log(2,30/.5)+2020), # for Δx 
	floor(Int,2*log(2,40000/700)+2020) # for Nₓ
end
...
begin
	...
	RadiationOceanModel(G::Grid) = 
		RadiationOceanModel(G, RadiationOceanModelParameters())
end;
...
α(T::Array{Float64,2}, model::RadiationOceanModel) = 
...
y_to_lat(y::Real; grid::Grid) = 
	π/2 * 0.5 * (y / grid.L + 1.0)

# ╔═╡ 2ace4750-2dbe-11eb-0074-0f3a7a929176
S_at(y::Float64; grid::Grid, S_mean::Float64) = 
...
function absorbed_solar_radiation(T::Array{Float64,2}, model::RadiationOceanModel)
	y = model.grid.y[:]
	S = S_at.(y; grid=model.grid, S_mean=model.params.S_mean)

	S .* (1 .- α(T, model)) ./ 4model.params.C
end
...
outgoing_thermal_radiation(T; C, A, B) = (A .- B .* (T)) ./ C
...
outgoing_thermal_radiation(T::Array{Float64,2}, model::RadiationOceanModel) = 
	outgoing_thermal_radiation(T, C=model.params.C, A=model.params.A, B=model.params.B)
...
function runtime(N)
	...
	Δt = 12*60*60
	test_sim = ClimateModelSimulation(model, linearT(G), Δt)
	
	@elapsed timestep!(test_sim)
end

# ╔═╡ af02d23e-2e93-11eb-3547-85d2aa07081b
plot([2^i for i ∈ 1:10], runtime, xaxis=:log2)
...
radiation_sim = let
	...
	# params = RadiationOceanModelParameters(S_mean=1000, A=210, κ=2e4)
	params = RadiationOceanModelParameters(ΔT=10, S_mean=1000, A=100)
...
# ╔═╡ 64e91290-436b-11eb-3bee-6b51e0eb27fa
RadiationOceanModelParameters()
...
@ initial_temp set at -50, -40, -10°C an equilibrium is reached(to ~-42.5°) \
@ initial_temp set at 0, 4, 10, 16°C an equilibrium is reached slowly, where top half of the globe is covered in ice & bottom half(till equator) remains relatively warm. with global mean Temp. at ~6°C \
@ initial_temp set at 20, 60, 65 or 1000°C an equilibrium is reached(to ~24.5°).
"""
...
S_1000_stable_states = -75

# ╔═╡ bed2c7e0-2d15-11eb-14e3-93f5d1b6f3a1
S_2000_stable_states = 107.7
...
# ╔═╡ d060dcde-46a1-11eb-00ab-bdabb6a07535
function run_sim_till_equilibrium!(sim; Δitr=1_000, ϵ=1e-3)
	Tᵢ = zeros(size(sim.T))
	Tᵢ₊₁ = ones(size(sim.T))
	
	while !≈(norm(Tᵢ₊₁ - Tᵢ), 0.0, atol=ϵ)
		Tᵢ .= sim.T
		for i in 1:Δitr
			timestep!(sim)
		end
...
function eq_T(S=1380, T_init_value=10; Δitr=1_000, ϵ=1e-3)
	grid = Grid(10, 6.e6)
	...
	model = RadiationOceanModel(grid, params, u, v)
	Δt = 400*60*60
	radiation_sim = ClimateModelSimulation(model, copy(T_init), Δt)
	
	run_sim_till_equilibrium!(radiation_sim, Δitr=Δitr, ϵ=ϵ)
end
...
eq_T.(1380, [-100, 0, 100], Δitr=5, ϵ=1e-5)

# ╔═╡ 7433ce40-46b1-11eb-3ceb-6b1fb0bc25ab
@benchmark eq_T(1380, 20, Δitr=5)

# ╔═╡ 749aba80-46be-11eb-0f3b-d1dbfb8fc83c
@benchmark run_sim_till_equilibrium!(radiation_sim)

# ╔═╡ 02f9310e-46c1-11eb-3838-3b51a5d73aad
@benchmark timestep!(radiation_sim)

# ╔═╡ b4ccac30-43c6-11eb-31c6-41c894228802
...
# ╔═╡ 70a91410-4765-11eb-2f0d-094a73048290
begin
	S_val= [range(1000,1300, length=3)..., range(1350,1565, length=8)..., range(1650,2000, length=3)...]
	T_val= [-50, 0, 50];
end
...
# ╔═╡ 20e56210-4758-11eb-2e06-fb97163e9897
equilibrium_Ts = ThreadsX.map(ST -> eq_T(ST[1], ST[2], Δitr=10), bifurcation_ST)

# ╔═╡ 9e653c70-2e9d-11eb-0af5-25dc140d5824
begin
	p= plot(S_val,
		transpose(reshape(equilibrium_Ts,:,length(S_val)));
		label=["-50°C" "0°C" "50°C"],legend=:topleft,
		color=["yellowgreen" "deepskyblue2" "orange"],lw=2,alpha=0.5) 
	scatter!(p,map(x->x[1],bifurcation_ST),equilibrium_Ts;
		xlabel="S Mean",ylabel="T equilibrium",color=:red,
		title="Bifurication of T equi Vs Smean",
		label="Equilibrium Temp",alpha=0.7)
	p
end

# ╔═╡ ca4665c0-47c6-11eb-26c7-41d7626dbd3c
(1750+1500)/2
...
$(todo("albedo_hypothesis_description"))
"""
