...
begin
	# Pkg.add(["PlutoUI", "Plots", "Pipe"])

	using Plots
	gr()
	using PlutoUI
	using Statistics, StatsBase, Random, Pipe
end

# ╔═╡ 449f74f0-0f60-11eb-0a11-570686781f1e
...
Base.:+(a::T, b::T) where T<:Coordinate = T(a.x+b.x, a.y+b.y)
...
collide_boundary(c::Coordinate, L::Number) = Coordinate(clamp(c.x, -L, L), clamp(c.y, -L, L))
...
trajectory(c::Coordinate, n::Int, L::Number) = accumulate((x, y)->collide_boundary(x+y, L), rand(possible_moves, n), init=c)
...
# ╔═╡ dbae3532-0c7b-11eb-031c-23b380496c05
try
	p = plot(ratio=1, size=(650,200))
	plot_trajectory!(p, trajectory(Coordinate(4,4), 1000, 5); c="black", lw=4)
	p
catch
end
...
# ╔═╡ 6a6e98f0-15e1-11eb-2aaa-09a37e787c40
...
begin
	mutable struct Agent <: AbstractAgent
		...
		position::Coordinate
		num_infected::Int
		Agent() = new(S, Coordinate(), 0)
		Agent(c::Coordinate) = new(S, c, 0)
		Agent(s::InfectionStatus, p::Coordinate, n::Int) = new(s, p, n)
	end
end
...
function initialize(N::Number, L::Number; agent_Type=Agent)
	agents = [agent_Type(S, Coordinate(rand(-L:L), rand(-L:L)), 0) for _ ∈ 1:N]
	
	infectious_agent = rand(agents)
	infectious_agent.status = I
	
	agents
end

# ╔═╡ c6b11500-0cb5-11eb-046e-ed380d01913f
get_position(agent) = agent.position |> make_tuple

# ╔═╡ 5c1373a0-0f49-11eb-04cf-e753780aecb0
with_terminal() do
	@code_warntype initialize(3, 10)
end
...
position(agent) = agent.position |> make_tuple # uncomment this line

# ╔═╡ 3097c29e-0cb8-11eb-2397-3f91d0e1869c
set_status!(agent, new_status) = agent.status = new_status

# ╔═╡ 25edef20-0cbb-11eb-260e-8ba9fa2e636a
drunk_move_agent!(agent, L::Int) = agent.position = collide_boundary(agent.position+rand(possible_moves), L)

# ╔═╡ 48d44a00-0cb8-11eb-2750-37a6850c0cfd
begin
	is_susceptible(agent) = agent.status == S
	is_infected(agent) = agent.status == I
end

# ╔═╡ 8ddf5840-0cc4-11eb-392f-590abcded328
function count_statuses(agents)
	s_count = i_count = r_count = 0
	
	for agent ∈ agents
		s_count += is_susceptible(agent)
		i_count += is_infected(agent)
		r_count += agent.status == R
	end
...
color(agent) = color(agent.status)
...
visualize(agents::Vector, L) = scatter(get_position.(agents), ratio=1, xlim=(-L, L), ylim=(-L, L), color=color.(agents), leg=false)
...
function interact!(agent::Agent, source::Agent, infection::CollisionInfectionRecovery)
	if get_position(agent) == get_position(source)
...
pandemic = CollisionInfectionRecovery(0.5, 1e-5)

# ╔═╡ 4e7fd58a-0a62-11eb-1596-c717e0845bd5
@bind k_sweeps Slider(1:1000, default=100, show_value=true)
...
# 	for p_infection ∈ .00001:.00002:1, p_recovery ∈ .00001:.00002:1
# 		infection = CollisionInfectionRecovery(p_infection, p_recovery)
		
# 		experiments = filter!(experimental_Rₑ->experimental_Rₑ>0, 
# 			[run_experiment_N_get_Rₑ!(initialize(N, L), L, infection) for num_exp ∈ 1:10])
# 		Rₑ = isempty(experiments) ? 0.0 : mean(experiments)
		
# 		if Rₑ > 1
# 			with_terminal() do
# 				println("p_infection: $p_infection, p_recovery: $p_recovery")
# 			end
# 			println("p_infection: $p_infection, p_recovery: $p_recovery")
# 			break
# 		end
...
causes_outbreak = CollisionInfectionRecovery( 0.657, 0.001) # even better is pandemic
...
function sir_mean_error_plot(simulations::Vector{<:NamedTuple})
	susseptible_agents = map(sim -> sim.S, simulations)
	p = plot(susseptible_agents |> mean, ribbon=susseptible_agents |> std, 
		c=1, label="S")
	infected_agents = map(sim -> sim.I, simulations)
	plot!(p, infected_agents |> mean, ribbon=infected_agents |> std, 
		c=2, label="I")
	recovered_agents = map(sim -> sim.R, simulations)
	plot!(p, recovered_agents |> mean, ribbon=recovered_agents |> std, 
		c=3, label="R")
end

# ╔═╡ b1b1afda-0a66-11eb-2988-752405815f95
need_different_parameters_because = md"""
May B the new parameter `L`, in our simulation, has something to do with it...
"""
...
begin
	...
	SocialAgent(status, position, num_infected) = SocialAgent(status, position, num_infected, rand(LinRange(.1, .5, 10)))
end
...
# already done some of it in Exercise 2 & some in SocialAgent initialization
...
# ╔═╡ 0f7fd030-1c53-11eb-3988-9b04c13f7e19
md"""
...
mutable struct SocialDistancingAgent <: AbstractAgent
	...
	SocialDistancingAgent(status, position, num_infected) = new(status, position, num_infected, rand())
	SocialDistancingAgent(status, position, num_infected, pₘ) = new(status, position, num_infected, pₘ)
end

# ╔═╡ d1664150-1d54-11eb-07c5-61d65dc61896
function initialize_social_distancing_agents(N::Number, L::Number; pₘ)
	agents = @pipe [Coordinate(i, j) for i ∈ -L:L, j ∈ -L:L] |> 
		sample!(_, Array{Coordinate, 1}(undef, N), replace=false) |> 
		SocialDistancingAgent.(S, _, 0, pₘ)
...
# ╔═╡ 79f99b90-1d51-11eb-0078-8b90e40fc9a4
function step!(agents::Vector{SocialDistancingAgent}, L::Number, infection::AbstractInfection)
	source_idx = findfirst(is_infected, shuffle!(agents))
	source = isnothing(source_idx) ? (return) : agents[source_idx]
	place_2_move, place_2_infect = @pipe rand(possible_moves, 2) |> collide_boundary.(_ .+ Ref(source.position), L)
	# "infection"
	agent_idx = findfirst(a->a.position==place_2_infect, agents)
	if !isnothing(agent_idx) 
		agent = agents[agent_idx]
		if is_susceptible(agent) && rand() <= infection.p_infection
			set_status!(agent, I)
			source.num_infected += 1
		end
	end
	# movement
	if rand() <= source.pₘ && is_infected(source) && !any(a->a.position==place_2_move, agents)
		source.position = place_2_move
	end
	# recovery
	rand() <= infection.p_recovery && set_status!(source, R)
	
	agents
end

# ╔═╡ 30d1fcd0-1aab-11eb-1aeb-678d2f39ec0e
...
let
	N = 100
	L = 20
	num_sweeps = k_sweep_max
	agents = initialize(N, L)
	s_counts, i_counts, r_counts = zeros(Int, num_sweeps), zeros(Int, num_sweeps), zeros(Int, num_sweeps)
	
	for k ∈ 1:num_sweeps
		sweep!(agents, L, pandemic)
		s_counts[k], i_counts[k], r_counts[k] = count_statuses(agents)
	end
	
	p = plot(s_counts, label="S")
	plot!(p, i_counts, label="I")
	plot!(p, r_counts, label="R")
end

# ╔═╡ e5040c9e-0a65-11eb-0f45-270ab8161871
begin
	function precompute_simulation(N::Number, L::Number, Tmax::Number, infection::AbstractInfection)

		x = initialize(N, L)
		intermediate_states = Vector{typeof(x)}(undef, Tmax)

		Ss, Is, Rs = zeros(Int, Tmax), zeros(Int, Tmax), zeros(Int, Tmax)

		for t ∈ 1:Tmax
			sweep!(x, L, infection)

			Ss[t], Is[t], Rs[t] = count_statuses(x)
			intermediate_states[t] = deepcopy(x)
		end

		(intermediate_states = intermediate_states, Ss = Ss, Is = Is, Rs = Rs)
	end
	function simulation(N::Number, L::Number, Tmax::Number, infection::AbstractInfection; agent_Type=Agent)

		agents = initialize(N, L, agent_Type=agent_Type)
		Ss, Is, Rs = zeros(Int, Tmax), zeros(Int, Tmax), zeros(Int, Tmax)

		for t ∈ 1:Tmax
			sweep!(agents, L, infection)

			Ss[t], Is[t], Rs[t] = count_statuses(agents)
		end

		(S = Ss, I = Is, R = Rs, agents=agents)
	end
end

# ╔═╡ 45e61f20-0ff7-11eb-02ae-0f2af1af45c8
@benchmark precompute_simulation(50, 40, 200, pandemic)

# ╔═╡ 2e397de0-0d95-11eb-0dce-033f7cb00f55
experiment1 = precompute_simulation(50, 40, 10000, pandemic);

# ╔═╡ af4628e0-0d8e-11eb-32bc-bfcc4fa93a2f
md"""
` Tmax: `$(@bind Tmax Slider(1:length(experiment1.intermediate_states), show_value=true))
"""

# ╔═╡ b29c31b0-0d8e-11eb-01ad-431899c59201
let
	t = Tmax
	left = visualize(experiment1.intermediate_states[t], 40)
    
	right = plot(xlim=(1,Tmax), ylim=(1,50), size=(600,300))
	plot!(right, 1:t, experiment1.Ss[1:t], color=color(S), label="S")
	plot!(right, 1:t, experiment1.Is[1:t], color=color(I), label="I")
	plot!(right, 1:t, experiment1.Rs[1:t], color=color(R), label="R")

	plot(left, right)
end

# ╔═╡ dcb6e120-1aa9-11eb-3cda-a74fe3e9b911
repeat_simulations(N, L, T, infection, num_simulations; agent_Type=Agent) = map(1:num_simulations) do _
	simulation(N, L, T, infection, agent_Type=agent_Type)
end

# ╔═╡ 56f150b0-1aaa-11eb-09de-1505e9cabc2f
repeat_simulations(N, L, T, pandemic, 50) |> sir_mean_error_plot

# ╔═╡ 4c8edef0-19c7-11eb-2822-19674251863a
function run_experiment_N_get_Rₑ!(agents, L, infection)
	source = (@view agents[findfirst(is_infected, agents)]) |> first

	while is_infected(source) && !all(is_infected, agents)
		sweep!(agents, L, infection)
	end

	R₀ = source.num_infected
	Sfraction = count(is_susceptible, agents) / length(agents)
	R₀*Sfraction
end

# ╔═╡ 695a1db0-1aad-11eb-3295-2ba9a8bbf3f4
let
	experiments = filter!(experimental_Rₑ->experimental_Rₑ>0, 
		[run_experiment_N_get_Rₑ!(initialize(100, 20), 20, causes_outbreak) for num_exp ∈ 1:100])
		Rₑ = isempty(experiments) ? 0.0 : mean(experiments)
end

# ╔═╡ 390deb40-0ccb-11eb-0b61-2bdb591d2f5b
let
    N = 50
    L = 40
...
let
	...
	global social_agents = initialize(N, L, agent_Type=SocialAgent)
	Ss, Is, Rs = zeros(Int, Tmax), zeros(Int, Tmax), zeros(Int, Tmax)
	...
	for t in 1:Tmax
		...
		Ss[t], Is[t], Rs[t] = count_statuses(social_agents)
	end
	
	plot(xlim=(1,Tmax), ylim=(1,N), size=(600,300))
	plot!(Ss, color=color(S), label="S")
	plot!(Is, color=color(I), label="I")
	plot!(Rs, color=color(R), label="R")
end

# ╔═╡ faec52a8-0a60-11eb-082a-f5787b09d88c
scatter(a->a.social_score, a->a.num_infected, social_agents, leg=false)
...
let
	...
	global social_agents_in_lockdown = initialize(N, L, agent_Type=SocialAgent)
	Ss, Is, Rs = zeros(Int, Tmax), zeros(Int, Tmax), zeros(Int, Tmax)
	
	for t in 1:Tmax
		t == lockdown_t &&
			(a->a.social_score*.1).(social_agents_in_lockdown)
		for _ ∈ 1:50N
			step!(social_agents_in_lockdown, L, pandemic)
		end

		Ss[t], Is[t], Rs[t] = count_statuses(social_agents_in_lockdown)
	end
	...
	plot!(Is, color=color(I), label="I")
	plot!(Rs, color=color(R), label="R")
	title!("lockdown after $lockdown_t time steps")
end
...
let
	...
	global social_distancing_agents = initialize_social_distancing_agents(N, L, pₘ=.25)
	Ss, Is, Rs = zeros(Int, Tmax), zeros(Int, Tmax), zeros(Int, Tmax)
	
	for t in 1:Tmax
		sweep!(social_distancing_agents, L, CollisionInfectionRecovery(.5, 5e-5))

		Ss[t], Is[t], Rs[t] = count_statuses(social_distancing_agents)
	end
...
# ╔═╡ 1b00ee00-1d5a-11eb-31bd-a78f302767ce
md"""
yup! physical distancing has a some noticable impact.
"""
...
# ╔═╡ d4bfd5de-1dff-11eb-3542-fbd746aa9c6d
TODO = todo = html"<span style='display: inline; font-size: 2em; color: purple; font-weight: 900;'>TODO</span>"

# ╔═╡ e9a717c0-1dff-11eb-2f8f-4b4b958f2c87
todo
...
# ╔═╡ 01769dc0-1e06-11eb-2bd1-555a68f62d31
