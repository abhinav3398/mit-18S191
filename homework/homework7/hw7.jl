...
# ╔═╡ 8d89ab10-2034-11eb-22ee-e17b34a8fd13
md"""
wall_center_X: $(@bind wall_center_X Slider(-5:.1:5; default=0, show_value=true)) __ wall_center_Y: $(@bind wall_center_Y Slider(-5:.1:5; default=0, show_value=true))

wall_dirⁿ_X: $(@bind wall_dirⁿ_X Slider(-5:.1:5; default=0, show_value=true)) __ wall_dirⁿ_Y: $(@bind wall_dirⁿ_Y Slider(-5:.1:5; default=0, show_value=true))
"""

# ╔═╡ 0906b340-19d3-11eb-112c-e568f69deb5d
test_wall = Wall(
	[wall_center_X, wall_center_Y],
	normalize([wall_dirⁿ_X, wall_dirⁿ_Y]),
)
...
(D = intersection_distance(photon, wall); D > ϵ ? Intersection(wall, D, photon.p + D*photon.l) : Miss())
...
interact(photon::Photon, hit::Intersection{Wall}) = 
Photon(hit.point, normalize!(reflect(photon.l, hit.object.normal)), photon.ior)
...
# ╔═╡ 181a89c0-2035-11eb-17a7-31658b06b81f
md"""
philip_center_X: $(@bind philip_center_X Slider(-5:.1:5; default=0, show_value=true)) __ philip_center_Y: $(@bind philip_center_Y Slider(-5:.1:5; default=0, show_value=true))

philip_dirⁿ_X: $(@bind philip_dirⁿ_X Slider(-5:.1:5; default=0, show_value=true)) __ philip_dirⁿ_Y: $(@bind philip_dirⁿ_Y Slider(-5:.1:5; default=0, show_value=true))
"""

# ╔═╡ 55187168-1c78-11eb-1182-ab4336b577a4
philip = Photon([philip_center_X, philip_center_Y], normalize([philip_dirⁿ_X, philip_dirⁿ_Y]), 1.0)
...
2. Our scene is just a `Vector` of objects, and we compute the intersection between the photon and every object.
...
# function intersection_by_Ian(photon::Photon, sphere::Sphere; ϵ=1e-6)
	...
	R = R₀ + t*ℓ
	t > 0 ? Intersection(sphere, t, R) : Miss()
end
...
minimum(intersection.(Ref(photon), objects))
...
# ╔═╡ 0ae7dcb0-20e1-11eb-0efa-cd3e27af4580
sig(t; ϵ) = ifelse(t > ϵ, :p, ifelse(t < -ϵ, :n, :z))

# ╔═╡ 12469c80-20e1-11eb-335a-c5fbf688c39a
function intersection_by_Ian(photon::Photon, sphere::Sphere; ϵ=1e-6)
# function intersection(photon::Photon, sphere::Sphere; ϵ=1e-3)
...
# ╔═╡ 687b17c0-20e1-11eb-1f9a-d967e1a04bde
# @benchmark intersection(Photon([2, 0], normalize([-1,0]), 1.0), test_sphere)

# ╔═╡ 79031e30-20e1-11eb-0d7e-5b38676c0736
# @benchmark intersection_by_Ian(Photon([2, 0], normalize([-1,0]), 1.0), test_sphere)
...
function refract(
		ℓ₁::Vector, n̂::Vector,
		old_ior, new_ior; ϵ=1e-3
	)
	
	r = old_ior / new_ior
	
	n̂_oriented = if -dot(ℓ₁, n̂) < -ϵ
		-n̂
	else
		n̂
	end
	...
	normalize(r * ℓ₁ + (r*c - sqrt(Complex(D))) * n̂_oriented)
end

# ╔═╡ 6f6bdfe0-2318-11eb-1961-4bdd24cb299a
sqrt(Complex(-1))*[1, 5] |> normalize
...
sphere_normal_at(p::Vector{Float64}, s::Sphere; ϵ=1e-3) = normalize!(p - s.center)
...
function interact(photon::Photon, hit::Intersection{Sphere})
	...
	new_ior = old_ior == sphere.ior ? 1.0 : sphere.ior
	n̂ = sphere_normal_at(p, sphere)
	ℓ₂ = refract(ℓ₁, n̂, old_ior, new_ior)
...
(hit = closest_hit(photon, objects); interact(photon, hit))

# ╔═╡ 1a43b70c-1ca3-11eb-12a5-a94ebbba0e86
trace(photon::Photon, scene::Vector{<:Object}, N) = 
...
# ╔═╡ 9d0b29b0-2273-11eb-2abd-557ecb73dc6f
photons = [Photon([-9.5, y], [1, 0], 1.0) for y ∈ -3:.5:3];

# ╔═╡ 29d3d5c0-1f9c-11eb-2156-a356abfbca7e
md"""
sphere_center_X: $(@bind sphere_center_X Slider(-5:.1:5; default=0, show_value=true)) __ sphere_center_Y: $(@bind sphere_center_Y Slider(-5:.1:5; default=0, show_value=true))

sphere ior: $(@bind sphere_ior Slider(0:.1:5; default=1.5, show_value=true))
sphere radius: $(@bind sphere_radius Slider(1:5; default=2, show_value=true))


iteration: $(@bind sim_test_N Slider(1:30; default=1, show_value=true))
"""
...
sphere_intersection = intersection(Photon([2, 0], normalize([-1,0]), 1.0), test_sphere);
...
let
	...
	plot_photon_arrow!(p, Photon([2, 0], normalize([-1,0]), 1.0), 4; label="Philip")
...
let
	scene = [test_sphere, box_scene...]
	p = plot_scene(scene, legend=false, xlim=(-11,11), ylim=(-11,11))
	
	for photon ∈ photons
		path = trace(photon, scene, sim_test_N)
...
$(TODO_note("remove bug of random variations of single touch at different positions"))
"""
