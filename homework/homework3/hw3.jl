...
student = (name = "Abhinav Lakhani", kerberos_id = "3398abhinav")
...
clean(text) = @pipe text |> unaccent |> lowercase |> filter(isinalphabet, _)
...
double_letters = alphabet[findall(!iszero, [sample_freq_matrix[i, i] for i ∈ 1:size(sample_freq_matrix, 1)])]
...
row_col_answer = md"""
sum of each row = $(sum(sample_freq_matrix, dims=1) ) \
sum of each column = $(sum(sample_freq_matrix, dims=2)' )

i don't know what this means but, \
sum of sum of each row == sum of sum of each row
"""
...
matrix_distance(A, B) = (A - B) .|> abs |> sum
...
function word_counts(words::Vector{T}) where T
	counts = Dict{T, Int}()
...
function completions_cache(grams::Vector{T}) where T
	cache = Dict{T, T}()
	
	for gram ∈ grams
		firstⁿ_words_of_gram, last_word = gram[1:end-1], gram[end]
		
		haskey(cache, firstⁿ_words_of_gram) ?
			push!(cache[firstⁿ_words_of_gram], last_word) :
			(cache[firstⁿ_words_of_gram] = [last_word])
	end
