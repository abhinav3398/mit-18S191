...
begin
	...
	using Plots
	gr()
	using PlutoUI, BenchmarkTools, PlotlyBase
end
...
euler_integrate_step(f′::Function, fa::Number, a::Number, h::Number) = h*f′(a+h) + fa
...
function euler_integrate(f′::Function, fa::Number, T::AbstractRange)
	...
	map(T) do t
		tmp = fa
		fa = euler_integrate_step(f′, fa, t, h)
		tmp
	end
end
...
md"""
β: $(@bind β Slider(0:0.01:1, default=0.5, show_value=true)) __
γ: $(@bind γ Slider(0:0.01:1, default=0.02, show_value=true))
"""

# ╔═╡ 4ebbf700-251e-11eb-308e-9b5e24ef0a58
md"""
...
> it should depend on the difference of consecutive prediction of x0. If the difference is small(say near to η), we're good or else keep going.  
or
...
hw4_results = collect.(zip(hw4_results_transposed...)) .|> Tuple
...
# ╔═╡ 494e7670-1ea8-11eb-2dc6-c7f74642711d
begin
	import Base: -
	-(a::T, b::T) where T <: Tuple{Float64,Float64,Float64} = a .- b
end

# ╔═╡ d217a4b6-12e8-11eb-29ce-53ae143a39cd
finite_difference_slope(f::Function, a, h=1e-3) = (f(a+h) - f(a)) / h

# ╔═╡ f0576e48-1261-11eb-0579-0b1372565ca7
finite_difference_slope(sqrt, 4.0, 5.0)

# ╔═╡ bd8522c6-12e8-11eb-306c-c764f78486ef
∂x(f::Function, a, b; h=1e-6) = finite_difference_slope(x -> f(x, b), a, h)
...
∂y(f::Function, a, b; h=1e-6) = finite_difference_slope(y -> f(a, y), b, h)
...
gradient(f::Function, a, b; h = 1e-6) = (∂x(f, a, b, h=h), ∂y(f, a, b, h=h))
...
function euler_SIR_step(β, γ, sir_0, h)
	s, i, r = sir_0
...
function euler_SIR(β, γ, sir_0, T; h = step(T))
...
# ╔═╡ f557ffd0-1da0-11eb-18a1-d9d72560408e
plot_sir!(plot(), sir_T, euler_SIR(β, γ, (0.99, 0.01, 0.00), sir_T))

# ╔═╡ a7f1829c-12e8-11eb-15a1-5de40ed92587
gradient_descent_1d_step(f, x0; η=0.01, h=1e-6) = x0 - η*finite_difference_slope(f, x0, η)
# 	slope = finite_difference_slope(f, x0, η)
	
# 	# slope != 0 && return x0 + -η*slope
# 	# 0
# end
...
function gradient_descent_1d(f, x0; η=0.01, max_steps=1000, atol=1e-4)
	old_x0 = 2x0
	
	while !≈(x0, old_x0, atol=atol) && max_steps > 0
		old_x0, x0 = x0, gradient_descent_1d_step(f, x0, η=η)
		max_steps -= 1
	end
...
let
	...
	gradient_descent_1d(x -> (x - 5)^2 - 3, 0.0)
end

# ╔═╡ 852be3c4-12e8-11eb-1bbb-5fbc0da74567
# function gradient_descent_2d_step(f, x0, y0; η=0.01, h=1e-6)
# 	f′ = gradient(f, x0, y0, h=h)
	
# 	!all(iszero, f′) && return (x0 - η*f′[1], y0 - η*f′[2])
# 	(0.0, 0.0)
# end
gradient_descent_2d_step(f, x0, y0; η=0.01, h=1e-6) = (f′ = gradient(f, x0, y0, h=h); (x0 - η*f′[1], y0 - η*f′[2]))

# ╔═╡ 8a114ca8-12e8-11eb-2de6-9149d1d3bc3d
function gradient_descent_2d(f, x0, y0; η=0.1, max_iter=1000, ϵ=1e-4)
	counter = 0
...
# @benchmark gradient_descent_2d(himmelbau, 1, -1)
@code_native gradient_descent_2d(himmelbau, 1, -1)

# ╔═╡ 0dea1f70-124c-11eb-1593-e535ab21976c
gauss(x, μ, σ) = (1 / (sqrt(2π) * σ)) * exp(-(x-μ)^2 / σ^2 / 2)

# ╔═╡ 2fc55daa-124f-11eb-399e-659e59148ef5
loss_dice(μ, σ) = Statistics.norm(gauss.(dice_x, μ, σ) .- dice_y)^2
...
found_μ, found_σ = gradient_descent_2d(loss_dice, 30, 1, ϵ=1e-4, η=2^5, max_iter=10_000)
...
1 - abs(stats_μ - found_μ)

# ╔═╡ 6faf4074-1266-11eb-1a0a-991fc2e991bb
stats_σ = sqrt(sum(dice_x.^2 .* dice_y) - stats_μ .^ 2)

# ╔═╡ e55d9c1e-1267-11eb-1b3c-5d772662518a
1 - abs(stats_σ - found_σ)

# ╔═╡ 754b5368-12e8-11eb-0763-e3ec56562c5f
loss_sir(β, γ; h=1e-2) = Statistics.norm(euler_SIR(β, γ, hw4_results[1], hw4_T, h=h) .- hw4_results)^2

# ╔═╡ 273b5170-1ea8-11eb-1a16-db651339dd61
(.1,.2,.3) - (.12, .23, .34)

# ╔═╡ ee20199a-12d4-11eb-1c2c-3f571bbb232e
loss_sir(guess_β, guess_γ, h=1)
...
found_β, found_γ = gradient_descent_2d((β, γ) -> loss_sir(β, γ, h=1), guess_β, guess_γ, ϵ=1e-2, η=3e-8, max_iter=10_000)
...
if !@isdefined(gradient)
	...
	let
		...
		if result isa Missing
			still_missing()
		elseif !(result isa Vector || result isa Tuple)
...
yup! for `sir_results`, it slowly reaches an equilibrium & after ~75 time steps there is no infexted individual(epidemic deminishes) & ~25% of populations is still suseptiblr(never got infected).

$(todo("find significance of β & γ. how do they affect sir curve/model from sir plot."))
"""
...
function gradient_2d_viz_2d(N_gradient_2d, x0, y0)
	...
	p = Plots.heatmap(-4:0.1:5, -4:0.1:4, himmelbau)
...
if run_3d_visualisation
	let
		...
		p = gradient_2d_viz_3d(N, x0_gradient_2d, y0_gradient_2d)
		gr()
