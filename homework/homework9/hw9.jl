...
# ╔═╡ 08210f70-3dc8-11eb-249b-1972cf3cd4cf
...
more -ve B means more heat reflaction by planet & also its inversly proportional to the time planet took to reach equilibrium.
"""
...
Positivefeedbacks: cause B to become less negative, reducing the efficiency with which Earth cools itself by radiating thermal energy to space, and thus amplifying warming.
"""
...
let
	B = -.2:.001:.2
	plot(B, ECS.(B=B), legend=false,
		 ylabel="temperature change [°C]", xlabel="B",)
end
...
histogram(ECS_samples, size=(600, 250), label=nothing, xlabel="temperature change [°C]", ylabel="samples", bins=200)
...
100sum(ECS_samples .> ECS(B=mean(B_samples))) / length(B_samples)
...
function temperature_response(CO₂::Function, B::Float64=-1.3)
	temperature_response_model = Model.EBM(14, 1850, 1, CO₂, B=B)
	Model.run!(temperature_response_model, 2100)
	temperature_response_model.T[end]
end
...
expected_double_CO2_year = let
	t[findfirst(CO₂ -> CO₂ >= 2Model.CO2_RCP85(t[1]), Model.CO2_RCP85.(t))]
end
...
prob_RCP26 = count(x -> x > Model.T0+2, temperature_response.(Model.CO2_RCP26, B_samples)) / length(B_samples)

# ╔═╡ 9ee2d840-3d40-11eb-057c-0f6eecaefc27
prob_RCP85 = count(x -> x > Model.T0+2, temperature_response.(Model.CO2_RCP85, B_samples)) / length(B_samples)
...
# ╔═╡ 6bc5a7f0-3db6-11eb-023a-17fbe490aa20
...
@bind log_CO₂ Slider(log10(CO2min):.01:log10(CO2max); show_value=false)

# ╔═╡ 4ce93200-3db8-11eb-2fb7-6359e9bf110d
CO₂ = 10^log_CO₂

# ╔═╡ de95efae-2675-11eb-0909-73afcd68fd42
Tneo = -48

# ╔═╡ 5f6f8400-3db9-11eb-3399-9bfc4cf11d1a
...
# ╔═╡ c9cba430-3dc5-11eb-2f36-47dac3423dca
equilibrium_temperature(CO₂) = (ebm = Model.EBM(Tneo, 0, 1, t -> CO₂); Model.run!(ebm, 1000); ebm.T[end])

# ╔═╡ 9eb07a6e-2687-11eb-0de3-7bc6aa0eefb0
co2_to_melt_snowball = let
	CO₂s = CO2min:10:CO2max
	CO₂s[findfirst(CO₂ -> (temp_at_equilibrium = equilibrium_temperature(CO₂); α(temp_at_equilibrium) == Model.α), CO₂s)]
end
