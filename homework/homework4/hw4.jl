...
# ╔═╡ 1c961a10-07bb-11eb-16d3-83be84a3ac9a
using Interact
...
student = (name = "Abhinav Lakhani", kerberos_id = "akshu3398@gmail.com")
...
# ╔═╡ 98dabe80-0c0e-11eb-0a24-e9a2d88e950d
html"""<style>
pluto-output {
    font-family: "Cascadia", cascadia;
}
"""
...
# ╔═╡ 9cab2930-07b5-11eb-2f08-bdfb8533b0f3
...
do_experiment(p, N) = [recovery_time(p) for _ ∈ 1:N]
...
function frequencies(values)
	freq = Dict{Int, Float64}()
	
	fraction = 1/length(values)
	for value ∈ values
		freq[value] = get(freq, value, 0) + fraction
	end
...
function frequencies_plot_with_mean(data)
	...
	base = bar(frequencies(data), alpha=.3)
	_mean = mean(data)
	vline!(base, [_mean], label="mean: $_mean")
...
# ╔═╡ 849a0fe0-07bb-11eb-2b3a-314db673faa2
begin
	p_interactive_slider = @bind p_interactive Slider(.01:.01:1, default = 0.1, show_value=true)
	N_interactive_slider = @bind N_interactive Slider(1:10:100_000, default = 100, show_value=true)
	
	md"""
	` p_interactive: `$p_interactive_slider 
	
	` N_interactive: `$N_interactive_slider
	"""
end
...
# ╔═╡ 6cbf20c0-07cc-11eb-0555-c5e2b5a7a30d
Pₙ(p, t) = p*(1-p)^(t-1) # according to probability theory
...
begin
	...
	plot!(Pₙ.(p_interactive, 1:maximum(experiment)), label="𝑝(1−𝑝)^(t−1)")
	title!("p=$p_interactive, N=$N_interactive")
end
...
let
	N = 10_000
	p = mean.(do_experiment.(.001:.001:1, N))
	plot(.001:.001:1, p, label="τ<p>")
	hline!([mean(p)], label="mean(τ) = $(mean(p))")
end
...
# ╔═╡ daab49b0-07d1-11eb-21c8-55c8897da913
Integer.([S, I, R])
...
set_status!(agent::Agent, new_status::InfectionStatus) = agent.status = new_status
...
# ╔═╡ cf24dc60-07d5-11eb-19c7-e1cb8b4783f3
0 + true, 1 + false

# ╔═╡ ad15d610-07d5-11eb-0da6-8b942d441ade
...
function sir_mean_plot(simulations::Vector{<:NamedTuple})
	...
	p = plot(map(sim -> sim.S, simulations) |> mean, c=1, label="S")
	plot!(p, map(sim -> sim.I, simulations) |> mean, c=2, label="I")
	plot!(p, map(sim -> sim.R, simulations) |> mean, c=3, label="R")
end
...
md"""
` p_infection: `$(@bind p_infection Slider(.01:.01:1, default = 0.02, show_value=true))  

` p_recovery: `$(@bind p_recovery Slider(.001:.001:1, default = 0.002, show_value=true))
"""
...
# ╔═╡ abc7b2c0-07f7-11eb-1944-673afb65d3ad
begin
	function counts(data)
		...
		ks, vs = keys(d) |> collect, values(d) |> collect
		p = sortperm(ks)
	...
	function infected_mean_error_plot(simulations::Vector{<:NamedTuple})
		T = length(first(simulations).S)

		num_infected_dist = map(sim -> sim.num_infected, simulations) .|> probability_distribution

		p = plot(num_infected_dist, legend = false, label=nothing)
		xlabel!(p, "Num infected")
		ylabel!(p, "frequency")
	end
end
...
# todo[find]: Rₒ, recovery time(i.e. time to reach steady state), & when does an epidemic occur
...
function simulation(N::Integer, T::Integer, infection::AbstractInfection)
	...
	S_counts, I_counts, R_counts = Int[], Int[], Int[]
	...
	return (S=S_counts, I=I_counts, R=R_counts, num_infected=map(a -> a.num_infected, agents))
end
...
begin
	...
	simulations = repeat_simulations(N, T, InfectionRecovery(p_inf, p_rec), num_simulations)
end
...
let
	...
	plot!(p, map(sim -> sim.I, simulations) |> mean, lw=3, label="mean")
...
# ╔═╡ abce1a30-07dd-11eb-321b-3b339bd8b9c4
repeat_simulations(N, T, InfectionRecovery(p_infection, p_recovery), num_simulations) |> sir_mean_plot

# ╔═╡ 5bc85b80-07e3-11eb-1059-2f03933c5af9
repeat_simulations(N, T, InfectionRecovery(p_infection, p_recovery), num_simulations) |> sir_mean_error_plot

# ╔═╡ 26e2978e-0435-11eb-0d61-25f552d2771e
repeat_simulations(N, T, InfectionRecovery(p_infection, p_recovery), num_simulations) |> infected_mean_error_plot
...
begin
	repeat_simulations(N, T, Reinfection(p_infection, p_recovery), num_simulations) |> sir_mean_error_plot
	title!("p_infection: $p_infection, p_recovery: $p_recovery")
end
